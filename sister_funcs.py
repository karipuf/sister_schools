...
def match_images(url1,url2,client,model='gemma3:12b'):
    
     imdat1=base64.b64encode(open(url1,"rb").read()).decode("utf-8")
     imdat2=base64.b64encode(open(url2,"rb").read()).decode("utf-8")

     try:
         res=client.chat.completions.create(model=model,messages=[{'role':'user',
                                                                   ...
                                                                   Please only return 'matchin' as True if the two objects are very clearly the same thing.
